...
public class LogRepositoryAspect {
    ...
    public void beforeRepository(JoinPoint joinPoint) {
        log.info("Repository {} starts to work with BD ",
    ...
    public void afterRepository(JoinPoint joinPoint, Object result) {
        ...
        log.info("Method {} of the repository {} received {}",
                joinPoint.getSignature().getName(),
                joinPoint.getSignature().getDeclaringTypeName(),
                result);
    }

    @Pointcut(value = "execution(public * org.springframework.data.repository.Repository+.*(..))")
    ...
    public void beforeCrudRepository(JoinPoint joinPoint) {
        if(joinPoint.getSignature().getName().equals("save")) {
            ...
            log.info("Repository starts to execute the method: {} ", joinPoint.getSignature().getName());
        }
    }
    ...
    public void afterCrudRepository(JoinPoint joinPoint, Object result) {
        if(joinPoint.getSignature().getName().equals("save")) {
            log.info("Repository saved the data in DB successfully.");
        } else {
            log.info("Repository ended executing the method: {} ", joinPoint.getSignature().getName());
        }
    }
